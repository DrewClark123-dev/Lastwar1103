...
def render_selection_boxes(col):
    ...
    if st.session_state.herometric_choice == 'Server':
        ...
        if 'selected_alliances' not in st.session_state:
            st.session_state.selected_alliances = ['OLDS','KOUS','DHCM','ASHH','NatA','Bytl','SHT1']
...
def print_server_chart(col, metric):
    ...
    server_chart = alt.Chart(all_servers_df).mark_line(point=True).encode(
        ...
        title=alt.TitleParams(text=f"Comparing Total Hero Power per Warzone  (10/5/25)", anchor='middle', fontSize=24),
        height=800
    ).interactive()
...
def print_alliance_chart(col, metric):
    ...
    server_chart = alt.Chart(all_alliances_df).mark_line(point=True).encode(
        ...
        title=alt.TitleParams(text=f"Comparing Total Hero Power per Alliance  (10/5/25)", anchor='middle', fontSize=24),
        height=800
    ).interactive()
    col.altair_chart(server_chart, use_container_width=True)
